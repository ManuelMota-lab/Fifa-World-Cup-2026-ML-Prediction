...
`elo_ratings_wc2026.csv` — 4,683 rows, 23 columns, UTF-8, comma-separated.
...
| `wc2026_exit_round` | string | How far the team went in the 2026 World Cup, on the **live-snapshot row only** (blank on year-end rows): `active` if still in, else the round eliminated — `group`, `R32`, `R16`, `QF`, `SF`, `final`, or `winner`. Updated each match day. |
